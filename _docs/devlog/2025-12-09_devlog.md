...
- **Before:** `#72777d` (4.48:1 contrast) — **FAILS WCAG AA**
- **After:** `--color-text-muted` = `#54595d` (7.0:1 contrast) — **PASSES**
...
- ❌ Block RGB/HSL values
- ✅ Require CSS variables for all colors
...
- ✅ Hero subtitle now readable (7.0:1 contrast)
- ✅ Future color issues prevented by linting
- ✅ Clear documentation for contributors
- ✅ Utility classes for consistent accessible text
...
1. **Hero subtitle nearly invisible** — Starlight's hero component used very low contrast text
2. **Content area too narrow** — Main content locked at 800px max-width
3. **No desktop breakpoints** — CardGrid only had mobile breakpoints
4. **Component descriptions using muted colors** — Several components used `--color-text-muted` which could be hard to read
...
- `TopicCard.astro` — description text
- `CompletenessMeter.astro` — count text
- `SeeAlso.astro` — description text
- `BlankSlate.astro` — description text
- `DecisionMatrix.astro` — confidence, explanation, method text
...
- **Tight but not cramped** — Reduced spacing by ~30-50% throughout
- **Consistent rhythm** — Using spacing scale consistently
- **Wikipedia-inspired** — Clean, efficient use of space
- **Mobile-first** — Base styles tight, desktop adds breathing room
...
✅ Hero is significantly more compact
✅ Title now fits on 2 lines instead of 3
...
- `src/styles/custom.css` — Hero overrides, header cleanup, container resets
...
1. **No `!important`** — User preference, using specificity and unlayered CSS instead
2. **Keep sidebar hidden** — Maintains Wikipedia aesthetic (Option A from audit)
3. **Keep `link` in sidebar config** — Sidebar is hidden anyway, no benefit to changing
...
## Session 18: THE GREAT STARLIGHT REMOVAL 🎉 (3:00 PM - 3:45 PM PST)

### The Problem

After hours of fighting Starlight's opinions (sidebar toggles, CSS overrides, component customizations), the fundamental realization hit:

> **The complexity isn't Astro — it's Starlight.**

Starlight is a docs theme with strong opinions. We spent the entire day fighting those opinions instead of building.

### The Decision

| Option | Effort | Result |
|--------|--------|--------|
| A: Keep Starlight, stop customizing | Low | Accept its defaults |
| B: Plain Astro, no Starlight | Medium | Full control |
| C: SvelteKit from scratch | High | Build everything |
| **Winner** | **B** | **Freedom** |

### What We Did

#### 1. Created Simple Base Layout

`src/layouts/Base.astro` — One file with everything:
- HTML structure
...
`astro.config.mjs` went from 78 lines of Starlight config to:
...
#### 3. Created Dynamic Content Route

`src/pages/[...slug].astro` — Renders all content from `src/content/docs/`:
...
#### 4. Created Simple Replacement Components

Replaced Starlight's fancy components with simple alternatives:

| Starlight Component | Our Replacement | Lines |
|---------------------|-----------------|-------|
| `<Tabs>` + `<TabItem>` | `details/summary` based | ~40 |
| `<Steps>` | CSS counter-based `ol` | ~45 |
| `<Aside>` | Colored callout box | ~40 |

Location: `src/components/simple/`

#### 5. Updated Content Imports

Changed 8 files from:
```javascript
...
| Build time | ~14s | **6.6s** |
| Config lines | 78 | **20** |
| CSS overrides | 266 lines fighting defaults | **0** (you own it) |
| Dependencies | Starlight + all its deps | MDX + Sitemap |
| Sidebar | Hours of fighting | **5 lines in layout** |
| Theme system | Complex override system | **CSS variables, done** |

### Files Created

| File | Purpose |
|------|---------|
| `src/layouts/Base.astro` | The one layout file you need |
| `src/pages/index.astro` | Simple homepage |
| `src/pages/[...slug].astro` | Dynamic content renderer |
| `src/components/simple/Steps.astro` | Numbered steps |
| `src/components/simple/Tabs.astro` | Tab container |
| `src/components/simple/TabItem.astro` | Tab item (uses details/summary) |
| `src/components/simple/Aside.astro` | Callout boxes |

### Files Deleted/Obsoleted

Everything in:
- `src/components/overrides/` — Starlight overrides
- `src/components/utilities/ForceLightTheme.astro` — Starlight theme hack
- `src/components/utilities/Empty.astro` — Starlight component hider
- `src/components/utilities/SidebarToggle.astro` — Hours of work, gone
- `src/styles/custom.css` — Most of it was Starlight overrides
...
src/
├── layouts/
│   └── Base.astro              ← ONE layout file (header, nav, footer, styles)
├── pages/
│   ├── index.astro             ← Homepage
│   └── [...slug].astro         ← Renders all content
├── content/
│   └── docs/                   ← Markdown content (unchanged)
│       ├── protocol/
│       ├── field-notes/
│       └── reports/
├── components/
│   └── simple/                 ← Simple component replacements
│       ├── Steps.astro
│       ├── Tabs.astro
│       ├── TabItem.astro
│       └── Aside.astro
└── content.config.ts           ← Simple content collection
```

### Lessons Learned

1. **Frameworks should help, not fight** — If you're spending hours overriding defaults, you chose the wrong tool

2. **Simpler is faster** — 6.6s builds vs 14s. Less code = less to break

3. **Own your layout** — One file with header, nav, footer, styles. No mystery

4. **Content collections are powerful** — Astro's `getCollection()` + `[...slug].astro` = automatic routing

5. **Don't add complexity for "someday"** — Starlight has features we never used (i18n, versioning, search index). Plain Astro has exactly what we need
...
- **Nav:** Right there in the layout
- **Styles:** Global styles in the layout
- **Add content:** Drop `.mdx` in `src/content/docs/`

No more fighting. Just building.

---

*Session 18 complete: 3:45 PM PST*
...
A Starlight docs site with Wikipedia aesthetic, fighting CSS overrides and sidebar issues.

### What Ended

A clean, simple Astro site with full control.
...
|---|---|---|
| Build time | 14s | 6.6s |
| Config complexity | 78 lines | 20 lines |
| CSS overrides | 266 lines | 0 |
| Hours fighting framework | Many | 0 |
| Satisfaction | 😤 | 🎉 |
...
## Session 19: Site Simplification & Polish

**Time:** ~3:40 PM - 4:40 PM PST
**Focus:** UI improvements, navigation restructure, disclaimers, password protection

### Summary

Major session focused on simplifying the site structure, improving visual design, and adding legal/access protections.

---

### Changes Made

#### 1. Card Visual Improvements
- Removed emojis from all TopicCards
- Moved all component styles from scoped to global (Base.astro)
- Added visual click affordance:
  - Blue link-colored titles
...
**Before:**
```
Home | Protocol | Field Notes | Reports
```

**After:**
```
[Site Title] | FAQ | Notes | Tools
```

- Removed "Home" nav item (site title serves as home link)
- Renamed `protocol/` → `faq/`
- Renamed `field-notes/` → `notes/`
- Merged `reports/` INTO `notes/`, then split templates to `tools/`
- Created `tools/` section for templates/checklists

#### 3. Disclaimer Improvements

**Footer disclaimer** with proper legal language:
- "Informational and educational purposes only"
- "Not a licensed financial advisor, CPA, or attorney"
- "All investments involve risk"
- "You assume full responsibility"

**New dedicated page:** `/disclaimer/`
- Full legal disclaimer
- Covers: No guarantees, Your responsibility, Limitation of liability, Third-party links

#### 4. Password Protection

Added client-side password gate:
- Password: `unlockmenow`
- Shows splash page on first visit
...
| File | Change |
|------|--------|
| `src/layouts/Base.astro` | Global styles, password gate, disclaimer |
| `src/pages/index.astro` | Updated nav links |
| `src/pages/disclaimer.astro` | New dedicated disclaimer page |
| `src/content/docs/faq/` | Renamed from protocol/ |
| `src/content/docs/notes/` | Renamed from field-notes/ |
| `src/content/docs/tools/` | New section for templates |
| `README.md` | Updated documentation |
| `src/components/molecules/TopicCard.astro` | Removed scoped styles |
| Multiple components | Moved styles to global |

---

### Commits

1. `refactor: remove Starlight, simplify to plain Astro`
2. `refactor: merge reports into field-notes, simplify nav`
3. `refactor: rename nav to FAQ/Notes/Tools, update README`
4. `fix: add visual click affordance to cards (blue title + arrow)`
5. `fix: add persistent disclaimer to header and footer of all pages`
6. `feat: improve disclaimer with proper legal language and dedicated page`
7. `feat: add password protection with splash gate`
...
```
src/content/docs/
├── faq/              # Core concepts (was: protocol)
│   ├── index.mdx
│   ├── introduction.mdx
│   ├── compound-interest.mdx
│   └── ... (10 articles)
├── notes/            # Research & observations (was: field-notes)
│   ├── index.mdx
│   ├── latest.mdx
│   └── ... (4 articles)
├── tools/            # Templates & calculators (new)
│   ├── index.mdx
│   ├── financial-autonomy-checklist.mdx
│   ├── decision-matrix-template.mdx
│   └── tax-optimization-guide.mdx
└── index.mdx         # Homepage
```
...
**Adding content:**
1. Write `.mdx` file in appropriate folder
2. Run `npm run ship "message"`
...
- [ ] Rename "FAQ" to something better (Wiki? Reference? Concepts?)
